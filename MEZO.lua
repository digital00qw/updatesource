...
ControllerAll = false
local ListSudos ={Sudo_Id,1833610203,1001132193,111111111111111}  
...
Status = 'DEV • BARLO'
elseif UserId == 111111111111111 then  
Status = 'DEV³'
...
msg.The_Controller = 1
elseif tonumber(msg.sender.user_id) == 111111111111111 then
...
return LuaTele.sendText(msg_chat_id,msg_id,'*هآي تع اشب شآآي • 🌝😂*',"md",false, false, false, false, reply_markup)
end
...
return LuaTele.sendText(msg_chat_id,msg_id,'*وانا بعشق آهلك يقلبو • ❤🧐*',"md",false, false, false, false, reply_markup)
end
...
local NamesBot = (Redis:get(MEZO.."MEZO:Name:Bot") or "كرستين •")
local BotName = {
...
if text == (Redis:get(MEZO.."MEZO:Name:Bot") or "كرستين •") then
local photo = LuaTele.getUserProfilePhotos(MEZO)
local ban = LuaTele.getUser(MEZO)
for Name_User in string.gmatch(ban.first_name, "[^%s]+" ) do
ban.first_name = Name_User
...
local CmdStart = '*\n ◉ أهلآ بك في بوت '..(Redis:get(MEZO.."MEZO:Name:Bot") or "كرستين •")..
